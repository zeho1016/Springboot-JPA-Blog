...
public class DummyControllerTest {
	...
	@Transactional // 함수 종료시 자동 commit   //save를 하지않아도 update가 된다. 
	@PutMapping("dummy/user/{id}")
	public User updateUser(@PathVariable int id, @RequestBody User requestUser) { // json데이터를 요청 => Java Object로 변환해서 받아줌
	...
	public User detail(@PathVariable int id) {
		// user/4를 찾으면 내가 데이터베이스에서 못찾아오게 되면 user가 null이 되는데
		// null값을 return 하게 되면 프로그램에 문제가 생김
		// 그래서 Optional로 User객체를 감싸서 가져올테니 null인지 아닌지 판단해서 return해
		...
		// 만약 자바 오브젝트를 리턴하게 되면 MessageConvert 가 Jackson 라이브러리를 호출해서
